...
def save_product(id,name, price, category, image_url, status='active'):
    products_sheet = get_worksheet('foodproducts')
    if products_sheet:
        products_sheet.append_row([id,name, float(price), category, status, image_url])
        st.success(f"บันทึกสินค้า '{name}' เรียบร้อยแล้ว")

def update_product(row_idx, id, name, price, category, status, image_url):
    products_sheet =get_worksheet('foodproducts')
    if products_sheet:
        try:
            products_sheet.update(f'A{row_idx+2}:F{row_idx+2}', [[id, name, float(price), category, status, image_url]])
