...
def show_dialogAddProd():
    ...
    if st.button("เพิ่มสินค้า",type="primary",key="add_product"):
        if name and price > 0 and image_url:
            db.save_product(id,name, price, category, image_url)
            st.rerun()
        else:
...
def show_dialogEditProd(row_idx,product):
    ...
    if product:
        with st.form(key=f"edit_form_{product['id']}"):
            ...
            edit_category = st.selectbox("หมวดหมู่", ["อาหารจานหลัก", "ของทานเล่น", "เครื่องดื่ม", "ของหวาน"], index=["อาหารจานหลัก", "ของทานเล่น", "เครื่องดื่ม", "ของหวาน"].index(product["category"]))
            ...
            if st.form_submit_button("บันทึก"):
                db.update_product(row_idx,edit_id, edit_name, edit_price, edit_category, edit_status, edit_image_url)
